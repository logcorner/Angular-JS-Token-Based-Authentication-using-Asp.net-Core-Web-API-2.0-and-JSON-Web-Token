...
var ProductService = (function () {
    function ProductService(http, authProfile, commonService) {
        this.http = http;
        this.authProfile = authProfile;
        this.commonService = commonService;
    }
    ProductService.prototype.getProducts = function () {
        var url = this.commonService.getBaseUrl() + '/product';
        var options = null;
        var profile = this.authProfile.getProfile();
        if (profile != null && profile != undefined) {
            var headers = new http_1.Headers({ 'Authorization': 'Bearer ' + profile.token });
